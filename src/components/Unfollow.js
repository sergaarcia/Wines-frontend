...
const Unfollow = () => {
  ...
  const handleUnfollow = async (e) => {
    ...
    const soapRequest = `
      <soapenv:Envelope xmlns:soapenv="http://www.w3.org/2003/05/soap-envelope" xmlns:ns="http://sos.etsiinf.upm.es" xmlns:ax21="http://model.sos.etsiinf.upm.es/xsd" xmlns:xs="http://www.w3.org/2001/XMLSchema">
        <soapenv:Header xmlns:wsa="http://www.w3.org/2005/08/addressing">
          <wsa:Action>urn:unfollow</wsa:Action>
    ...
    try {
      const response = await axios.request({
        method: 'post',
        url: 'http://localhost:8080/axis2/services/WineSocialUPM/unfollow',
        data: soapRequest,
        headers: {
      ...
      if (responseNode && responseNode.textContent === 'true') {
        ...
        setMessage('Unfollow failed: ' + (responseNode ? responseNode.textContent : 'Unknown error'));
      }
    } catch (err) {
      setMessage('Unfollow failed: ' + err.message);
    }
  };
  ...
  return (
    <Container maxWidth="sm">
      <Box sx={{ mt: 5 }}>
        <Typography variant="h4" gutterBottom>Unfollow</Typography>
