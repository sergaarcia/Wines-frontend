  ...
  const handleAddUser = async (e) => {
    ...
    const soapRequest = `
      <soapenv:Envelope xmlns:soapenv="http://www.w3.org/2003/05/soap-envelope" xmlns:ns="http://sos.etsiinf.upm.es" xmlns:ax21="http://model.sos.etsiinf.upm.es/xsd" xmlns:xs="http://www.w3.org/2001/XMLSchema">
        <soapenv:Header xmlns:wsa="http://www.w3.org/2005/08/addressing">
          <wsa:Action>urn:addUser</wsa:Action>
    ...
    try {
      const response = await axios.request({
        method: 'post',
        url: 'http://localhost:8080/axis2/services/WineSocialUPM/addUser',
        data: soapRequest,
        headers: {
      ...
      const passwordNode = xmlDoc.getElementsByTagName('ns1:pwd')[0];

      if (responseNode && responseNode.textContent === 'true') {
        setMessage(`User added successfully with password: ${passwordNode.textContent}`);
        incrementMessageId();
      } else {
        setMessage('Add User failed: ' + (responseNode ? responseNode.textContent : 'Unknown error'));
      }
    } catch (err) {
      setMessage('Add User failed: ' + err.message);
    }
  };
